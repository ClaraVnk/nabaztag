...
| **phase8-rom** (experimental) | 108 KB | -6.4 KB | Everything in `full` + `--gc-sections` **plus** the Phase 8 stack: (1) 4 portal HTML pages live in C-side flash ROM rendered via `OPpageRender`; (2) `mkwav 8000 1 16` precomputed at build time and inlined as a 46-byte literal (delete the `fun mkwav` body); (3) `dump`/`dumpscan` debug trace utilities reduced to identity-passthrough (DEBUG_VM is off, UART is unwired); (4) all 92 `Secho{ln} "literal"` debug-print calls replaced with `nil` (output went nowhere). Smallest .sim of all modes. Frees ~10 KB of vmem RAM at runtime and ~17 KB of bytecode + globals. Untested on hardware as of 2026-06-01 — flash `full` first, treat `phase8-rom` as the experimental headroom slice. |
