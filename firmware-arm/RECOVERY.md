...
| **phase8-rom** (experimental) | 108 KB | -6.4 KB | Everything in `full` + `--gc-sections` **plus** the Phase 8 stack of 7 source-level slices: (1) 4 portal HTML pages live in C-side flash ROM rendered via a new `OPpageRender` opcode; (2) `mkwav 8000 1 16` precomputed and inlined as a 46-byte literal; (3) `dump`/`dumpscan` reduced to identity (UART unwired so the prints go to a sink nobody reads); (4) all 92 `Secho{ln} "literal"` calls replaced with `nil`; (5) user echo helpers `MACecho`/`SEQecho`/`IPecho` collapsed to identity (passthrough preserved for callers using their return); (6) every dynamic-arg `Secho`/`Secholn`/`Iecho`/`Iecholn` opcode keyword deleted — they're confirmed stack passthroughs in vinterp.c; (7) `dump`/`dumpscan` callsites inlined and their now-dead definitions dropped. **Smallest .sim of all modes (220 762 B)**, -17 944 B vs minimal/rescue, -5 896 B vs lean. Frees ~10 KB of vmem RAM at runtime. Every slice is idempotent and composable in the mode's step list. Untested on hardware as of 2026-06-01 — flash `full` first, treat `phase8-rom` as the experimental headroom slice. |
