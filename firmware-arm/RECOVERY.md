...
| **phase8-rom** (experimental) | 112 KB | -2.6 KB | Everything in `full` + `--gc-sections` **plus** the Phase 8 stack: (1) 4 portal HTML pages live in C-side flash ROM rendered via `OPpageRender`; (2) `mkwav 8000 1 16` precomputed at build time and inlined as a 46-byte literal (delete the `fun mkwav` body); (3) `dump`/`dumpscan` debug trace utilities reduced to identity-passthrough (DEBUG_VM is off, UART is unwired). Smallest .sim of all modes. Frees ~10 KB of vmem RAM at runtime. Untested on hardware as of 2026-06-01 — flash `full` first, treat `phase8-rom` as the experimental headroom slice. |
